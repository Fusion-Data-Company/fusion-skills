...
export default async function handler(req:VercelRequest,res:VercelResponse){
 ...
 const token=crypto.randomUUID();const {rows:[job]}=await pool.query(`UPDATE fs_jobs SET delivery_status='sending',lease_token=$1,lease_until=now()+interval '45 seconds',attempts=attempts+1 WHERE id=(SELECT j.id FROM fs_jobs j JOIN fs_accounts a ON a.tenant_id=j.tenant_id WHERE j.delivery_status='pending' AND j.available_at<=now() AND a.access_until>now() AND a.billing_status IN ('active','trialing') AND a.billing_invoice_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM fs_payment_holds h WHERE h.invoice_id=a.billing_invoice_id AND h.held) ORDER BY j.created_at FOR UPDATE OF j SKIP LOCKED LIMIT 1) RETURNING *`,[token]);
 if(!job)return res.json({claimed:false});
 const {rows:[entitled]}=await pool.query("SELECT 1 FROM fs_accounts a WHERE a.tenant_id=$1 AND a.access_until>now() AND a.billing_status='active' AND a.billing_invoice_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM fs_payment_holds h WHERE h.invoice_id=a.billing_invoice_id AND h.held)",[job.tenant_id]);
 if(!entitled){await pool.query("UPDATE fs_jobs SET delivery_status='pending',lease_until=NULL,lease_token=NULL,updated_at=now() WHERE id=$1 AND lease_token=$2",[job.id,token]);return res.json({claimed:false,held:true});}
